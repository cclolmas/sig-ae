...
import { Estoque as EstoqueType, UnidadeEscolar } from '@/types'
...
export function Estoque() {
  ...
  const fetchEstoques = async (): Promise<void> => {
    try {
      const response = await fetch('/api/estoque')
      if (response.ok) {
        const data: EstoqueType[] = await response.json()
        setEstoques(data)
      }
    } catch (error) {
  ...
  const getStatusEstoque = (saldo: number): { label: string; color: string } => {
  ...
  const getStatusValidade = (dataValidade?: string): { label: string; color: string } | null => {
